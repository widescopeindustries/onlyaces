...
const marketStats = [
  {
    ...
    value: "48M+",
    detail:
      "NGF said total golf participation exceeded 48 million in 2025, with more than 29 million on-course golfers.",
  },
  {
    label: "Texas footprint",
    value: "715",
    detail:
  ...
  },
  {
    ...
    value: "~20 PGA",
    detail:
      "D Magazine reported around 20 PGA Tour pros in and around Dallas, with almost 40 total pros across North Texas.",
  },
];
...
const comparableSignals = [
  {
    name: "Sink or Win",
    summary:
      "Texas floating-green challenge now marketing across DFW. Useful signal of local appetite, but still water-dependent and venue-partner driven.",
  },
  {
    name: "TheMoneyShot.golf",
    summary:
      "Automated $10k floating-island format with leaderboard and video hooks. Strong signal for the content layer, but still built around water hazards.",
  },
  {
    name: "Chip 2 Win",
    summary:
      "Mobile $10k event product that explicitly asks hosts for water. Strong proof of prize-shot demand, weak proof of land-based flagship scale.",
  },
];

const buildoutProfiles = [
  {
    name: "Lean pilot",
    range: "$40k-$90k",
    summary:
      "A private proof rig to validate the 111-yard shot, target design, pricing, scoring, and content flow before polishing the flagship.",
  },
  {
    name: "Public flagship MVP",
    range: "$150k-$350k+",
    summary:
      "A real first site with conditioned bays, safety netting, cameras, electrical/data work, and customer-facing brand polish.",
  },
];
...
const diligenceQuestions = [
  "Lock the MVP hardware stack for scoring, cameras, and target verification.",
  "Separate lean pilot cost from flagship-grade build cost instead of blending them.",
  "Prove customer throughput and pricing without fantasy assumptions.",
  "Decide which parts need flagship polish on day one and which can stay modular.",
];

const partnerStructure = [
  {
    title: "Public operator lane",
    body:
      "The flagship likely performs better with a visible operator face, local identity, and credibility rather than a purely autonomous kiosk posture.",
  },
  {
    title: "Concept-originator lane",
    body:
      "The originating value is the land-based unlock, flagship logic, pricing posture, programming vision, and replication blueprint. That role can stay strategically central without needing public visibility.",
  },
  {
    title: "Best first structure",
    body:
      "Operator-led externally, concept-led internally: defined pre-development involvement, flagship participation, and a path into future rollout if the model validates.",
  },
];

const coreDefinition = [
  "A premium skill-based hole-in-one challenge built around one iconic 111-yard shot.",
  "A visible jackpot model designed to pull repeat attempts while keeping the math in the operator's favor.",
  "A land-based water-target format that preserves the emotional hook without needing a lake or pond.",
];
...
const offerToChris = [
  {
    title: "An unlock, not a random idea",
    body:
...
export default function Home() {
  return (
    <main className="page-shell pb-20">
      ...
      <section className="container-frame pt-10 md:pt-14">
        <div className="glass-panel rounded-[36px] px-5 py-6 md:px-8 md:py-8">
          <div className="eyebrow">Demand signals</div>
          <div className="mt-5 grid gap-4 md:grid-cols-4">
            {marketStats.map((stat) => (
              <article
                key={stat.label}
                className="rounded-[26px] border border-line bg-panel-strong px-4 py-5"
              >
                <p className="text-xs uppercase tracking-[0.24em] text-ink-soft">
                  {stat.label}
                </p>
      ...
      <section className="container-frame pt-10 md:pt-14">
        <div className="grid gap-5 md:grid-cols-[1.05fr_0.95fr]">
          <article className="glass-panel rounded-[32px] px-5 py-6 md:px-7">
            <div className="eyebrow">Stateside read</div>
            <h2 className="mt-4 font-display text-4xl tracking-[-0.04em]">
              The U.S. market is not empty. It is still trapped by water.
            </h2>
            <p className="mt-4 max-w-2xl text-base leading-7 text-ink-soft">
              The strongest stateside analogs validate the prize-shot demand.
              They also reinforce the same pattern: water dependence, venue
              dependence, or pop-up deployment instead of a repeatable
              land-based flagship.
            </p>
          </article>
      ...
      <section className="container-frame pt-10 md:pt-14">
        <div className="grid gap-5 md:grid-cols-[0.95fr_1.05fr]">
          <article className="glass-panel rounded-[32px] px-5 py-6 md:px-7">
            <div className="eyebrow">What Only Aces is</div>
            <h2 className="mt-4 font-display text-4xl tracking-[-0.04em]">
              A portable premium challenge business wearing a golf wrapper
            </h2>
            <ul className="deck-list mt-6">
              {coreDefinition.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </article>
          ...
          <article className="glass-panel rounded-[32px] px-5 py-6 md:px-7">
            <div className="eyebrow">What it becomes with Chris</div>
            <h2 className="mt-4 font-display text-4xl tracking-[-0.04em]">
              A real flagship instead of an idea trapped in your head
            </h2>
            <ul className="deck-list mt-6">
              {withChris.map((item) => (
                <li key={item}>{item}</li>
      ...
      <section className="container-frame pt-10 md:pt-14">
        <div className="glass-panel rounded-[36px] px-5 py-6 md:px-8 md:py-8">
          <div className="eyebrow">What we are offering Chris</div>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {offerToChris.map((item) => (
              <article
                key={item.title}
                className="rounded-[26px] border border-line bg-panel-strong px-5 py-5"
              >
      ...
      <section className="container-frame pt-10 md:pt-14">
        <div className="glass-panel rounded-[36px] px-5 py-6 md:px-8 md:py-8">
          <div className="eyebrow">Capex posture</div>
          <div className="mt-5 grid gap-5 md:grid-cols-2">
            {buildoutProfiles.map((profile) => (
              <article
                key={profile.name}
                className="rounded-[26px] border border-line bg-panel-strong px-5 py-5"
              >
                ...
                <div className="metric-value mt-4">{profile.range}</div>
                <p className="mt-3 text-sm leading-6 text-ink-soft">
                  {profile.summary}
                </p>
              </article>
      ...
      <section className="container-frame pt-10 md:pt-14">
        <div className="grid gap-5 md:grid-cols-[1.05fr_0.95fr]">
          <article className="glass-panel rounded-[32px] px-5 py-6 md:px-7">
            <div className="eyebrow">Internal structure</div>
            <h2 className="mt-4 font-display text-4xl tracking-[-0.04em]">
              The visible flagship and the originating concept do not need to
              occupy the same lane
            </h2>
            <p className="mt-4 max-w-3xl text-base leading-7 text-ink-soft">
              This project can be public-facing on the operator side while still
              protecting the strategic value of the concept originator behind the
              scenes. That distinction matters if the business needs a clean
              public face without weakening the originating role.
            </p>
            <div className="mt-6 grid gap-4">
              {partnerStructure.map((item) => (
                <article
                  key={item.title}
                  className="rounded-[24px] border border-line bg-panel-strong px-4 py-5"
                >
          ...
          <aside className="glass-panel rounded-[32px] px-5 py-6 md:px-7">
            <div className="eyebrow">Role framing</div>
            <h2 className="mt-4 font-display text-4xl tracking-[-0.04em]">
              Founding concept partner
            </h2>
            <p className="mt-4 text-base leading-7 text-ink-soft">
              The strongest internal framing is not consultant, advisor, or idea
              guy. It is concept originator and strategic architect of the U.S.
              flagship adaptation, with defined involvement in MVP shaping,
              customer posture, programming logic, and replication planning.
            </p>
            <div className="faint-rule my-6" />
            <ul className="deck-list">
              <li>Protect the land-based unlock from getting diluted into a generic range concept</li>
              <li>Help shape flagship design, customer experience, and operating posture</li>
              <li>Participate in rollout logic if the flagship validates the broader model</li>
            </ul>
          </aside>
        </div>
      </section>
        ...
        <div className="glass-panel rounded-[36px] px-5 py-6 md:px-8 md:py-8">
          ...
          <div className="mt-5 grid gap-5 md:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-base leading-7 text-ink-soft">
                The thesis now has enough support to stop sounding speculative.
                The next move is converting the strongest arguments into a real
                pilot path, a credible flagship budget posture, and a clean
                founder conversation if Chris wants to move.
              </p>
            </div>
            <ul className="deck-list">
